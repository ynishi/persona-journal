...
- **CLI `import-fs --dedup-by-line` flag for `NamedIndex`-mode kinds.** Lines whose trimmed form already exists in the kind's `first_line_cache` are skipped (`summary.skipped`) instead of being appended as duplicates. For entries-mode kinds the flag is a no-op since the existing `uname` collision path already dedups by `(year_month, seq)`. Backed by `Journal::find_named_index_by_name` (new public API) and `Db::find_uname_by_first_line` (new internal primitive). Unit tests: `run_import_fs_named_index_dedup_by_line_skips_existing_on_second_run` / `run_import_fs_named_index_dedup_by_line_appends_only_new_lines` / `run_import_fs_entries_dedup_by_line_is_noop`.
...
- **`Journal::append_named_index` — new public API for `NamedIndex`-mode kinds.** `pub fn append_named_index(&self, persona: &str, kind: &str, name: &str, body: &str) -> Result<String>` inserts one row into a `NamedIndex`-mode kind and returns its `uname`. `seq_in_kind` is generated by the existing `next_seq()` helper (`YYYY-MM_NNNNNN` format), so the DDL `CHECK(uname = kind || '/' || seq_in_kind)` constraint is satisfied without schema changes. `name` is stored in `entries.first_line_cache`. The function rejects non-`NamedIndex` kinds with `Error::Invalid("append_named_index supports named_index mode only")`. `say()` is unchanged and continues to reject `NamedIndex` inputs — the two functions share no common code path. Unit tests: `append_named_index_basic_inserts_row` / `append_named_index_rejects_entries_mode` / `append_named_index_allows_duplicate_name`.

- **CLI `import-fs` now supports `NamedIndex`-mode kinds.** `run_import_fs` detects the kind's mode at startup and dispatches to either the existing entries path (`run_import_fs_entries`, walking a directory) or the new named-index path (`run_import_fs_named_index`, reading a file line by line). For `NamedIndex` mode: blank lines and lines whose trimmed form starts with `#` are skipped (`summary.skipped`); each remaining line is inserted as an independent row via `Journal::append_named_index` (`summary.written`). `--force-override` is accepted but has no effect for `NamedIndex` (auto-generated `seq_in_kind` never conflicts). `--dry-run` prints `[would import] persona=… kind=… line=…` without writing. `ImportSummary` struct and `Cmd::ImportFs` enum are unchanged. Unit tests in `main.rs`: `run_import_fs_named_index_inserts_lines` / `run_import_fs_named_index_skips_blank_and_comment` / `run_import_fs_named_index_allows_duplicate_lines`.
